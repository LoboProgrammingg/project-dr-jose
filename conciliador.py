...
# --- MÓDULOS 1 e 2: LEITURA DE ARQUIVOS (sem alterações) ---
...
def processar_banco_ofx(caminho_ofx: str) -> pd.DataFrame:
    if not os.path.exists(caminho_ofx): raise FileNotFoundError(f"Arquivo OFX não encontrado em: {caminho_ofx}")
    ...
    for transacao in extrato.transactions:
        transacoes.append({
            "Data": transacao.date.date(), "Historico_Banco": transacao.memo,
            "Valor_Banco": float(transacao.amount), "Codigo_Banco": transacao.id
        })
    if not transacoes: return pd.DataFrame(columns=["Data", "Historico_Banco", "Valor_Banco", "Codigo_Banco"])
...
def gerar_relatorio_definitivo(df_banco: pd.DataFrame, df_egestor: pd.DataFrame, arquivo_saida: str):
    ...
    # A adição de reset_index() é crucial para manter os índices originais para referência
    df_merged = pd.merge(
...
if __name__ == "__main__":
    ofx_banco_path = "sicoob_2025_07_09_09_46_01-converted (3).ofx"
    csv_egestor_path = "sem_alteracao.csv"
    relatorio_path = 'sem_alteracao_data.xlsx'
