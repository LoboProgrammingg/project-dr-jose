...
class DetalheAnoView(LoginRequiredMixin, ListView):
    ...
    context_object_name = (
        'planilhas_upload'  # Contexto para planilhas de upload
    )
    paginate_by = 5
    ...
    def get_context_data(self, **kwargs):
        ...
        planilhas_nativas = PlanilhaNativa.objects.filter(
            ano_referencia=self.ano_obj
        )
        if search_query:
            planilhas_nativas = planilhas_nativas.filter(
...
def upload_planilha(request, ano):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(
                    request,
                    'planilhas/planilha_form.html',
                    {'form': form, 'ano': ano_obj},
                )
    ...
    return render(
        request, 'planilhas/planilha_form.html', {'form': form, 'ano': ano_obj}
    )
...
class EditarPlanilhaView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            'planilhas:detalhe_ano',
            kwargs={'ano': self.object.ano_referencia.ano},
        )
...
def download_planilha(request, pk):
    ...
    response[
        'Content-Disposition'
    ] = f'attachment; filename="{arquivo.name.split("/")[-1]}"'
    return response
...
def criar_planilha_nativa(request, ano):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            planilha.colunas = [
                col.strip() for col in colunas_str.split(',') if col.strip()
            ]
    ...
    return render(
        request,
        'planilhas/planilha_nativa_form.html',
        {'form': form, 'ano': ano_obj},
    )
...
def editar_planilha_nativa(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            planilha.save()
            return JsonResponse(
                {'status': 'success', 'message': 'Planilha salva com sucesso!'}
            )
...
def download_planilha_nativa(request, pk):
    ...
    if csv_data:
        ...
        response[
            'Content-Disposition'
        ] = f'attachment; filename="{planilha.nome}.csv"'
        return response
