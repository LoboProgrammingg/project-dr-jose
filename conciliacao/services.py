...
def limpar_valor_csv(valor_str: str) -> float | None:
    ...
    if not isinstance(valor_str, str):
        return None
    sinal = (
        -1 if '(-)' in valor_str or valor_str.strip().startswith('-') else 1
    )
    # Remove R$, pontos de milhar e outros caracteres não numéricos
    valor_limpo = re.sub(
        r'[^\d,]', '', str(valor_str).replace('.', '').replace('R$', '')
    )
...
def processar_banco_ofx(arquivo_ofx_em_memoria) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            ...
            print(
                f'ERRO CRÍTICO: Não foi possível decodificar o arquivo OFX com as codificações testadas. Erro: {e}'
            )
            return (
                pd.DataFrame()
    ...
    if not conteudo_texto:
        print(
            'ERRO CRÍTICO: O conteúdo do arquivo OFX está vazio ou não pôde ser lido.'
        )
        return pd.DataFrame()
    ...
    try:
        ...
        print(
            f'Processamento do OFX concluído. Encontradas {len(df_banco)} transações.'
        )
...
def carregar_egestor_csv(arquivo_csv_em_memoria) -> pd.DataFrame:
    ...
    try:
        ...
        if 'Descrição' not in df.columns:
            print(
                "AVISO: Coluna 'Descrição' não encontrada no CSV. Uma coluna de histórico vazia será usada."
            )
            df['Descrição'] = ''
        ...
        df['Data'] = pd.to_datetime(
            df['Data'], errors='coerce', format='%d/%m/%Y'
        )
        df['Valor_eGestor'] = (
            df['Valor_eGestor'].astype(str).apply(limpar_valor_csv)
        )
        df['Valor_eGestor'] = pd.to_numeric(
            df['Valor_eGestor'], errors='coerce'
        )
...
def gerar_dataframe_conciliacao(
    df_banco: pd.DataFrame, df_egestor: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    if df_banco.empty or df_egestor.empty:
        print(
            'AVISO: Um dos DataFrames está vazio. Não é possível realizar a conciliação.'
        )
    ...
    for _, row in divergencias.iterrows():
        ...
        status = (
            ...
            if banco_presente and gestor_presente
                ...
                'PENDENTE (APENAS NO BANCO)'
                if banco_presente
                else 'PENDENTE (APENAS NO GESTOR)'
            )
        )
        ...
        historico_banco = (
            row['Historico_Banco_Banco'] if banco_presente else ''
        )
        historico_gestor = (
            row['Historico_eGestor_Gestor'] if gestor_presente else ''
        )
...
def criar_arquivo_excel(df_relatorio: pd.DataFrame) -> io.BytesIO:
    ...
    df_relatorio_formatado['Data'] = pd.to_datetime(
        df_relatorio_formatado['Data']
    ).dt.strftime('%d/%m/%Y')

    with pd.ExcelWriter(
        output_em_memoria, engine='xlsxwriter', datetime_format='dd/mm/yyyy'
    ) as writer:
        df_relatorio_formatado.to_excel(
            writer,
            sheet_name='Painel de Conciliação',
            index=False,
            header=False,
            startrow=1,
        )
        ...
        status_conciliado_format = workbook.add_format(
            {'bg_color': '#C6EFCE', 'font_color': '#006100', 'border': 1}
        )
        status_divergencia_format = workbook.add_format(
            {'bg_color': '#FFC7CE', 'font_color': '#9C0006', 'border': 1}
        )
        status_pendente_format = workbook.add_format(
        ...
        worksheet.conditional_format(
            f'{status_col_letter}2:{status_col_letter}{len(df_relatorio_formatado)+1}',
            {
                'type': 'cell',
                'criteria': '==',
                'value': '"Conciliado"',
                'format': status_conciliado_format,
            },
        )
        worksheet.conditional_format(
            f'{status_col_letter}2:{status_col_letter}{len(df_relatorio_formatado)+1}',
            {
                ...
                'format': status_divergencia_format,
            },
        )
        worksheet.conditional_format(
            f'{status_col_letter}2:{status_col_letter}{len(df_relatorio_formatado)+1}',
