...
try:
    ...
    print("Por favor, certifique-se de que 'services.py' e 'test_conciliation.py' estão na mesma pasta.")
    exit()
...
def run_annual_reconciliation():
    ...
    if not os.path.exists(OFX_FILE_PATH):
        ...
        print("Por favor, renomeie o seu arquivo OFX para 'extrato_anual.ofx' ou ajuste o nome no script.")
        return

    if not os.path.exists(CSV_FILE_PATH):
        print(f"ERRO: Arquivo CSV não encontrado em '{CSV_FILE_PATH}'")
        print("Por favor, certifique-se de que 'teste2.csv' está na pasta correta.")
        return
    ...
    try:
        ...
        if df_banco.empty:
            print("AVISO: Nenhum dado de transação foi extraído do arquivo OFX. Verifique o arquivo.")
            # Continuamos mesmo assim para ver se há dados no CSV.
        else:
            print(f"-> Sucesso! {len(df_banco)} transações lidas do extrato bancário.")
    ...
    try:
        ...
        if df_egestor.empty:
            print("AVISO: Nenhum dado foi extraído do arquivo CSV. Verifique o arquivo.")
        else:
            print(f"-> Sucesso! {len(df_egestor)} lançamentos lidos da planilha.")
    ...
    try:
        ...
        print(f"\n--- SUCESSO! ---")
        print(f"O relatório de conciliação foi salvo em: '{os.path.abspath(OUTPUT_EXCEL_FILE)}'")
